...
public class DefaultLogFilterAndRule implements LogFilterAndRule {
    ...
    public Map doAnalysis(Map<String, String> header, String log) {
        ...
        // add line number
        resultMap.put("_lnum", header.get(TaildirSourceConfigurationConstants.READ_LINE_NUMBER));
        // add timestamp
        if (timeStampField == TIMESTAMP_NEED) {
            resultMap.put("_timestamp", header.get(TaildirSourceConfigurationConstants.READ_TIMESTAMP));
        }
