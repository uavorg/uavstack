...
/**
 * AppExprAdaptor description: for APP_RESP expr's adaption
 *
 */
public class AppExprAdaptor extends AbstractExprAdaptor{


    public AppExprAdaptor(String cName, String feature, IActionEngine engine) {
    ...
    public String convertInstance(String instance, String type) {
        ...
        switch (type) {
            case CLIENT_RESP:
                ...
                if(strs.length==3) {
                    instance = "http://"+strs[0]+"*---"+strs[1]; 
                }else {
                    instance = null ;
                }
                break;
            //not true when one server have several Apps 
            case URL_RESP:
